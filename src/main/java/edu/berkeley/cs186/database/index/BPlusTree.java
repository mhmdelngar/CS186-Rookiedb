...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        for (i = 0; i < leafNode.getKeys().size(); i++) {
            if (key.compareTo((leafNode).getKeys().get(i)) == 0) {
                index = i;
            }
    ...
    public Iterator<RecordId> scanAll() {
        ...
        BPlusTreeIterator bPlusTreeIterator = new BPlusTreeIterator(leafNode);
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        int index = -1;
        int left = 0;
        int right = leafNode.getKeys().size() - 1;

        while (left <= right) {
            int mid = left + (right - left) / 2;

            if (key.compareTo(leafNode.getKeys().get(mid)) == 0) {
                index=mid;
                break;
            } else if (key.compareTo(leafNode.getKeys().get(mid)) > 0) {
                left = mid + 1;
            } else {
                right = mid - 1;
            }
        }
        BPlusTreeIterator bPlusTreeIterator =new BPlusTreeIterator(leafNode ,key,index);
        return bPlusTreeIterator;
    }
...
//        if ( root instanceof  InnerNode){
        BPlusNode bPlusNode1 = root.fromBytes(metadata, bufferManager, lockContext, root.getPage().getPageNum());
        System.out.println("added key" + key);

        Optional<Pair<DataBox, Long>> optionalPair = bPlusNode1.put(key, rid);
        if (optionalPair.isPresent()) {
            List<Long> children = new ArrayList<>();
            List<DataBox> keys = new ArrayList<>();

            keys.add(optionalPair.get().getFirst());
            children.add(bPlusNode1.getPage().getPageNum());
...
//        System.out.println("added key" + key);
        ...
        Optional<Pair<DataBox, Long>> optionalPair = bPlusNode1.bulkLoad(data, fillFactor);
        if (optionalPair.isPresent()) {
            List<Long> children = new ArrayList<>();
            List<DataBox> keys = new ArrayList<>();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator(LeafNode leafNode, DataBox key, int index) {
            this.leafNode = leafNode;
            this.key = key;
            this.index = index;
            if (index == -1) {
            ...
            boolean greaterThanKey=false;
                ...
                if (index < leafNode.getRids().size() ) {
                    ...
                    if (key ==null){
                        ...
                        greaterThanKey=leafNode.getKeys().get(index).compareTo(key) >= 0;
                    }
        ...
        public RecordId next() {
            ...
            index++;
            return leafNode.getRids().get(index - 1);
